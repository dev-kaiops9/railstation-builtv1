...
<div class="sidebar flex flex-col p-4" id="sidebar-menu">
    ...
    <div class="mb-6">
        ...
        <select
            ...
            <option value="{{ $name }}" {{ session('station') == $name ? 'selected' : '' }}>
                {{ $name }}
            </option>            
            @endforeach
        </select>
    ...
    <nav class="flex-grow">
        <ul>
            ...
            <x-nav-li :href="route('railibrary')" id="menu-railibrary" icon="fas fa-book"
                :active="request()->routeIs('railibrary')">
                RaiLibrary
            </x-nav-li>
